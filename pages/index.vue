...
export default {
  ...
  async startChat({ $axios }, murmel_id) {
    firebase
      .auth()
      .signInAnonymously()
    ...
    firebase.auth().onAuthStateChanged(function(user) {
      if (user) {
        ...
        chat._chat.createRoom('Chat', 'private', async roomId => {
          let success = true
          if (success) {
            const user = await getUserOrRequestFromServer()
            ...
            const res = await this.$axios.post(`/murmel/${murmel_id}/chat_room`, data, {
                    headers: { 'Access-Control-Allow-Origin': '*' },
                    auth: {
                        "username": user.userId,
                        "password": user.accessKey
...
.radar {
  ...
  #test-mood-2 {
    ...
    transform: translate3d(-50%, -50%, 0) rotate(360deg) translateX(80%)
      rotate(-360deg);
  }
  #test-mood-4 {
    // transform: container: put it in the middle of the radar, rotate, move outside (percent of full radius), rotate back (all aligned)
    transform: translate3d(-50%, -50%, 0) rotate(270deg) translateX(20%)
      rotate(-270deg);
  }

  #test-mood-5 {
    // transform: container: put it in the middle of the radar, rotate, move outside (percent of full radius), rotate back (all aligned)
    transform: translate3d(-50%, -50%, 0) rotate(90deg) translateX(40%)
      rotate(-90deg);
  }

  #test-mood-3 {
    // transform: container: put it in the middle of the radar, rotate, move outside (percent of full radius), rotate back (all aligned)
    transform: translate3d(-50%, -50%, 0) rotate(180deg) translateX(70%)
      rotate(-180deg);
  }

  #test-mood-6 {
    // transform: container: put it in the middle of the radar, rotate, move outside (percent of full radius), rotate back (all aligned)
    transform: translate3d(-50%, -50%, 0) rotate(320deg) translateX(85%)
      rotate(-320deg);
  }
